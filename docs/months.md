...
**`week_start`** — `sunday` or `monday`. Sunday by default. It moves where the
row breaks and nothing else: the week numbers stay ISO, and the weekend stays
Saturday and Sunday, which is what `clocks` reckons too. Anything
unrecognised is the default rather than a stopped panel.

**`timezone`** — an IANA name such as `Asia/Tokyo`. Empty means this machine's
own zone, which is what `clocks` shows.
