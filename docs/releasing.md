...
merge the release PR
        ...
        └─► tag-release.yml   sees the manifest names a version with no
                    ...
                    └─► release.yml   three targets built and tested,
                                      ...
                                      to a GitHub Release, and packed
...
**`NPM_TOKEN` is not optional**, once a release is meant to reach npm.
Create the `@stealth-factory` org on npm (the unscoped name `terminal-toys`
is a different package and cannot be this one), then an automation token
with permission to publish under that org, and store it as the `NPM_TOKEN`
repository secret. The publish job fails without it rather than leaving a
GitHub release whose version npm has never heard of.
...
intended failure: the token or the `@stealth-factory` org is missing, and
nothing was published on either side. Fix the secret, re-dispatch
`release.yml` at that tag. A version already on npm is skipped rather
than republished, so a retry after npm succeeded and the GitHub step
failed will finish the release.
