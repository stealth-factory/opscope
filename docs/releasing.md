...
lock disagreeing with the manifests. The three-platform `release.yml`
pull-request trigger is commented out; restore it when the PAT is in
place, and before making those checks required. Editing an existing bot
PR does not change its author: close it so the next run opens a new one.
