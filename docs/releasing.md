...
The three-platform `release.yml` pull-request trigger is commented out:
approving it only rehearses the tagged build, at the cost of a click and
a second set of binaries. Restore it when the PAT is in place, and before
making those checks required. `release-pr.yml` still runs
`cargo metadata --locked` on the bumped tree before offering it — the one
thing that step can get wrong is leaving the lock disagreeing with the
manifests. Editing an existing bot PR does not change its author: close
it so the next run opens a new one.
