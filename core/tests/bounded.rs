...
fn a_command_that_never_answers_is_given_up_on() {
    ...
    let got = opscope_core::run(&["sleep", "30"], 2);
...
fn a_command_that_answers_comes_back_with_its_output() {
    let got = opscope_core::run(&["echo", "hello"], 5).expect("echo");
...
fn a_command_that_fails_is_an_error_not_empty_output() {
    ...
    assert!(opscope_core::run(&["false"], 5).is_err());
}

#[test]
fn a_command_that_is_not_installed_says_so() {
    let got = opscope_core::run(&["definitely-not-a-real-binary-xyz"], 5);
    assert!(got.is_err());
}
