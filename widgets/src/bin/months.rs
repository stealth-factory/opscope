...
fn main() {
    tc::maybe_help(include_str!("months_help.txt"));
    ...
    loop {
        ...
        let foot: Vec<String> = tc::pack_hints(&hints(&p), w - 2, "  ")
            .into_iter()
            .map(|l| format!(" {}", l))
            .collect();
    ...
    fn no_row_of_the_frame_overflows_the_pane() {
        ...
        for w in 12..400usize {
            for note in [None, Some("timezone \"Antarctica/DumontDUrville\" is not in the database - reckoned on this machine's zone instead")] {
                let rows = frame(w, today, view, WeekStart::Sunday, "America/Argentina/ComodRivadavia (UTC-3)", note, &p);
                for row in &rows {
                    assert!(
                        cells(row) <= w,
                        "width {} drew a row of {}: {:?}",
                ...
                // And the footer, which is packed the same way.
                for line in tc::pack_hints(&hints(&p), w.saturating_sub(2), "  ") {
                    assert!(cells(&line) + 1 <= w, "width {} footer {:?}", w, line);
                }
            }
        }
    }
